...
**Discovery** (Path Patching, Wang et al 2022, Goldowsky-Dill et al 2023): for each of the 300 box tracking questions, construct a coresponding noise task with random query, box labels, and objects, and see for which "paths between nodes" patching over from the noisy run to the original run has the largest detrimental "effect".
	- the **path** between heads refers to the following:
