...
Digression: according to Geva et al 2021, you can view MLP's as key-value memories, where the row vectors of the MLP\_in are the keys, which when matched, activate a given neuron. And when that neuron is activated, the row vectors of the MLP\_out are the values that it adds.
