...
- key thing here: we only cache the c\_t^KV, the latent.
  - because we’ll later do q^TK \= smth **W\_q^T W\_UK** c\_t^KV, so we never have to instantiate the keys
...
- using fixed W^(UK) matrix, we generate d\_h different keys and values
...
- have to cache the k\_t^R’s now, but that’s fine because it’s smaller than the normal key
...
Trained softmax(I\_t, s) KL loss with the actual dense attention values
...
- that second term is just a Monte carlo estimate of the KL divergence between π\_old and π\_theta according to the samples
- model learns from its mistakes (A\_i, t \< 0) the most, don’t want it to train unstably on the mistakes where it’s very off-policy
